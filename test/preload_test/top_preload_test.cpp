...
struct Verifier : public sc_module {
  ...
  [[noreturn]] void verification() {
    ...
    NC_REPORT_TIMED_INFO(name(), "add_module should be preload")
    val = 0;
    ...
    while (true) {
      ...
      NC_REPORT_TIMED_INFO(
          name(), "Configure mixed_module (with inner_mul_module preloaded)")
      rz.configure_module<mixed_module>();
      val = 0;

      // inner module is preload, only wait for 5ns
